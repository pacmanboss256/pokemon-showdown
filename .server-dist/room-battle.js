 ...
 class RoomBattle extends RoomGames.RoomGame {
	...
	checkActive() {
		...
		if (this.ended || !this.started) {
			active = false;
		} else if (!_optionalChain([this, 'access', _3 => _3.p1, 'optionalAccess', _4 => _4.active])) {
			active = false;
		} else if (!_optionalChain([this, 'access', _5 => _5.p2, 'optionalAccess', _6 => _6.active])) {
			active = false;
		} else if (this.playerCap > 2) {
			if (!_optionalChain([this, 'access', _7 => _7.p3, 'optionalAccess', _8 => _8.active])) {
				active = false;
			} else if (!_optionalChain([this, 'access', _9 => _9.p4, 'optionalAccess', _10 => _10.active])) {
				active = false;
			}
		}
	...
	async onEnd(winner) {
		...
		if (this.replaySaved || Config.autosavereplays) {
			...
			if (_optionalChain([uploader, 'optionalAccess', _11 => _11.connections, 'access', _12 => _12[0]])) {
		...
		if (_optionalChain([parentGame, 'optionalAccess', _13 => _13.onBattleWin])) {
			// @ts-ignore
	...
	onLeave(user, oldUserid) {
		...
		if (_optionalChain([player, 'optionalAccess', _14 => _14.active])) {
	...
	addPlayer(user, team, rating = 0) {
		...
		if (_optionalChain([user, 'optionalAccess', _15 => _15.inRooms, 'access', _16 => _16.has, 'call', _17 => _17(this.roomid)])) this.onConnect(user);
		return player;
	}
	...
	start() {
		...
		const suspectTest = _optionalChain([Chat, 'access', _18 => _18.plugins, 'access', _19 => _19['suspect-tests'], 'optionalAccess', _20 => _20.suspectTests, 'access', _21 => _21[this.format]]);
		if (suspectTest) {
		...
		try {
			...
			if (battle) {
				for (const side of battle.sides) {
					if (_optionalChain([side, 'optionalAccess', _22 => _22.requestState])) {
 ...
 *********************************************************/
 ...
 const PM = new _lib.ProcessManager.StreamProcessManager(module, () => new RoomBattleStream()); exports.PM = PM;
