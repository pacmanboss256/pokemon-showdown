 ...
 */
 ...
 const commands = {
	...
	forcerename(target, room, user) {
		...
		if (_optionalChain([room, 'optionalAccess', _55 => _55.roomid]) !== 'staff') this.privateModAction(`${targetUser.name} ${forceRenameMessage}`);
		...
		Rooms.global.notifyRooms(
			['staff'],
			`|html|${roomMessage}` + _lib.Utils.html`<span class="username">${targetUser.name}</span> ${rankMessage} ${forceRenameMessage}`
		);
	...
	async namelock(target, room, user, connection, cmd) {
		...
		if (_optionalChain([targetUser, 'optionalAccess', _56 => _56.namelocked])) return this.errorReply(`User '${targetUser.name}' is already namelocked.`);
		const {privateReason, publicReason} = this.parseSpoiler(target);
		const reasonText = publicReason ? ` (${publicReason})` : `.`;
		this.privateGlobalModAction(`${_optionalChain([targetUser, 'optionalAccess', _57 => _57.name]) || userid} was ${week ? 'week' : ''}namelocked by ${user.name}${reasonText}`);
		...
		// Automatically upload replays as evidence/reference to the punishment
		if (_optionalChain([room, 'optionalAccess', _58 => _58.battle])) this.parse('/savereplay forpunishment');
	...
	unnamelock(target, room, user) {
		...
		const targetUser = Users.get(target);
		let reason = '';
		if (_optionalChain([targetUser, 'optionalAccess', _59 => _59.namelocked])) {
	...
	groupchatban(target, room, user, connection, cmd) {
		...
		for (const roomid of createdGroupchats) {
			...
			const participants = _optionalChain([targetRoom, 'access', _60 => _60.warnParticipants, 'optionalCall', _61 => _61(
	...
	blacklistname(target, room, user) {
		...
		const duplicates = targets.filter(userid => (
			...
			_optionalChain([Punishments, 'access', _62 => _62.roomUserids, 'access', _63 => _63.nestedGet, 'call', _64 => _64(room.roomid, userid), 'optionalAccess', _65 => _65[0]]) === 'BLACKLIST'
		));
