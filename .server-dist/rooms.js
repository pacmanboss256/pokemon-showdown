 ...
 class GameRoom extends BasicRoom {
	...
	async uploadReplay(user, connection, options) {
		...
		const [success] = await LoginServer.request('prepreplay', {
			...
			inputlog: _optionalChain([battle, 'access', _45 => _45.inputLog, 'optionalAccess', _46 => _46.join, 'call', _47 => _47('\n')]) || null,
		});
		if (success) battle.replaySaved = true;
		if (_optionalChain([success, 'optionalAccess', _48 => _48.errorip])) {
 ...
 const Rooms = {
	...
	createBattle(formatid, options) {
		...
		if (privacySetter.size) {
			...
			} else if (!options.tour || (_optionalChain([room, 'access', _49 => _49.tour, 'optionalAccess', _50 => _50.allowModjoin]))) {
