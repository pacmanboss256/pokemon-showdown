...
const ticketPages = {
 ...
 const commands = {
	...
	helpticket: {
		...
		close(target, room, user) {
			...
			if (!_optionalChain([ticket, 'optionalAccess', _21 => _21.open]) || (ticket.userid !== user.id && !user.can('lock'))) {
		...
		ban(target, room, user) {
			...
			for (const userObj of affected) {
				...
				if (_optionalChain([targetTicket, 'optionalAccess', _22 => _22.open])) targetTicket.open = false;
		...
		unban(target, room, user) {
			...
			this.checkCan('lock');
			target = toID(target);
			const targetID = _optionalChain([Users, 'access', _23 => _23.get, 'call', _24 => _24(target), 'optionalAccess', _25 => _25.id]) || target ;
			...
			_optionalChain([Users, 'access', _26 => _26.get, 'call', _27 => _27(target), 'optionalAccess', _28 => _28.popup, 'call', _29 => _29(`${user.name} has ticket unbanned you.`)]);
		},
 ...
 const punishmentfilter = (user, punishment) => {
	...
	if (_optionalChain([helpRoom, 'optionalAccess', _30 => _30.game, 'optionalAccess', _31 => _31.gameid]) !== 'helpticket') return;
