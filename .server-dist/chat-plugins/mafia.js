...
class Mafia extends Rooms.RoomGame {
	...
	lynch(userid, target) {
		...
		if (this.getHammerValue(target) <= lynch.trueCount) {
			// HAMMER
			this.sendDeclare(`Hammer! ${target === 'nolynch' ? 'Nobody' : _lib.Utils.escapeHTML(name)} was lynched!`);
	...
	unlynch(userid, force = false) {
		...
		if (!player && this.dead[userid] && this.dead[userid].restless) player = this.dead[userid];
		if (!_optionalChain([player, 'optionalAccess', _6 => _6.lynching])) return this.sendUser(userid, `|error|You are not lynching anyone.`);
