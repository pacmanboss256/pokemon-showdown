 ...
 class LadderStore {
	...
	async updateRating(p1name, p2name, p1score, room) {
		...
		try {
			...
			room.addRaw(_lib.Utils.html`${p1name}'s rating: ${oldelo} &rarr; <strong>${elo}</strong><br />(${reasons})`);
			let minElo = elo;
			...
			if (!reasons.startsWith('-')) reasons = '+' + reasons;
			room.addRaw(_lib.Utils.html`${p2name}'s rating: ${oldelo} &rarr; <strong>${elo}</strong><br />(${reasons})`);
	 ...
	 */
	 calculateElo(previousUserElo, score, foeElo) {
