 ...
 const Abilities = {
	...
	baddreams: {
		...
		onResidual(pokemon) {
			...
			for (const target of pokemon.side.foe.active) {
				if (!_optionalChain([target, 'optionalAccess', _2 => _2.hp])) continue;
	...
	frisk: {
		onStart(pokemon) {
			for (const target of pokemon.side.foe.active) {
				if (!_optionalChain([target, 'optionalAccess', _5 => _5.hp])) continue;
				if (target.item) {
	...
	galewings: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _6 => _6.type]) === 'Flying' && pokemon.hp === pokemon.maxhp) return priority + 1;
		},
		name: "Gale Wings",
		rating: 3,
		num: 177,
	...
	naturalcure: {
		onCheckShow(pokemon) {
			...
			for (const curPoke of pokemon.side.active) {
				...
				if (!_optionalChain([curPoke, 'optionalAccess', _11 => _11.status])) {
	...
	owntempo: {
		...
		onHit(target, source, move) {
			if (_optionalChain([move, 'optionalAccess', _12 => _12.volatileStatus]) === 'confusion') {
	...
	pastelveil: {
		...
		onSetStatus(status, target, source, effect) {
			...
			if (_optionalChain([(effect ), 'optionalAccess', _13 => _13.status])) {
		...
		onAllySetStatus(status, target, source, effect) {
			if (!['psn', 'tox'].includes(status.id)) return;
			if (_optionalChain([(effect ), 'optionalAccess', _14 => _14.status])) {
				const effectHolder = this.effectData.target;
	...
	pickpocket: {
		onAfterMoveSecondary(target, source, move) {
			if (source && source !== target && _optionalChain([move, 'optionalAccess', _15 => _15.flags, 'access', _16 => _16['contact']])) {
	...
	poisontouch: {
		...
		onModifyMove(move) {
			if (!_optionalChain([move, 'optionalAccess', _17 => _17.flags, 'access', _18 => _18['contact']]) || move.target === 'self') return;
			if (!move.secondaries) {
				move.secondaries = [];
			}
	...
	prankster: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _19 => _19.category]) === 'Status') {
	...
	serenegrace: {
		...
		onModifyMove(move) {
			...
			if (_optionalChain([move, 'access', _20 => _20.self, 'optionalAccess', _21 => _21.chance])) move.self.chance *= 2;
		},
		name: "Serene Grace",
		rating: 3.5,
		num: 32,
	...
	tangledfeet: {
		...
		onModifyAccuracy(accuracy, target) {
			if (typeof accuracy !== 'number') return;
			if (_optionalChain([target, 'optionalAccess', _23 => _23.volatiles, 'access', _24 => _24['confusion']])) {
	...
	triage: {
		onModifyPriority(priority, pokemon, target, move) {
			if (_optionalChain([move, 'optionalAccess', _25 => _25.flags, 'access', _26 => _26['heal']])) return priority + 3;
		},
		name: "Triage",
		rating: 3.5,
		num: 205,
	...
	waterveil: {
		...
		onSetStatus(status, target, source, effect) {
			if (status.id !== 'brn') return;
			if (_optionalChain([(effect ), 'optionalAccess', _29 => _29.status])) {
