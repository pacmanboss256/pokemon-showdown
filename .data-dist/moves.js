...
*/
 ...
 const Moves = {
	...
	assist: {
		...
		onHit(target) {
			const noAssist = [
				'assist', 'banefulbunker', 'beakblast', 'belch', 'bestow', 'bounce', 'celebrate', 'chatter', 'circlethrow', 'copycat', 'counter', 'covet', 'destinybond', 'detect', 'dig', 'dive', 'dragontail', 'endure', 'feint', 'fly', 'focuspunch', 'followme', 'helpinghand', 'holdhands', 'sshield', 'matblock', 'mefirst', 'metronome', 'mimic', 'mirrorcoat', 'mirrormove', 'naturepower', 'phantomforce', 'protect', 'ragepowder', 'roar', 'shadowforce', 'shelltrap', 'sketch', 'skydrop', 'sleeptalk', 'snatch', 'spikyshield', 'spotlight', 'struggle', 'switcheroo', 'thief', 'transform', 'trick', 'whirlwind',
			];
	...
	metalburst: {
		...
		onModifyTarget(targetRelayVar, source, target, move) {
			const lastDamagedBy = source.getLastDamagedBy(true);
			if (_optionalChain([lastDamagedBy, 'optionalAccess', _31 => _31.position]) !== undefined) {
	...
	mirrormove: {
		...
		onTryHit(target, pokemon) {
			const move = target.lastMove;
			if (!_optionalChain([move, 'optionalAccess', _32 => _32.flags, 'access', _33 => _33['mirror']]) || move.isZ || move.isMax) {
				return false;
			}
	...
	psychicterrain: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _37 => _37.hasItem, 'call', _38 => _38('terrainextender')])) {
					return 8;
				}
				return 5;
			},
			...
			onStart(battle, source, effect) {
				if (_optionalChain([effect, 'optionalAccess', _39 => _39.effectType]) === 'Ability') {
	...
	pursuit: {
		...
		onModifyMove(move, source, target) {
			if (_optionalChain([target, 'optionalAccess', _40 => _40.beingCalledBack]) || _optionalChain([target, 'optionalAccess', _41 => _41.switchFlag])) move.accuracy = true;
		},
		onTryHit(target, pokemon) {
	...
	reflect: {
		...
		condition: {
			duration: 5,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _42 => _42.hasItem, 'call', _43 => _43('lightclay')])) {
					return 8;
				}
				return 5;
			},
	...
	shelltrap: {
		...
		onTryMove(pokemon) {
			if (!_optionalChain([pokemon, 'access', _46 => _46.volatiles, 'access', _47 => _47['shelltrap'], 'optionalAccess', _48 => _48.gotHit])) {
				this.attrLastMove('[still]');
	...
	spitup: {
		...
		basePower: 0,
		basePowerCallback(pokemon) {
			if (!_optionalChain([pokemon, 'access', _49 => _49.volatiles, 'access', _50 => _50['stockpile'], 'optionalAccess', _51 => _51.layers])) return false;
	...
	suckerpunch: {
		...
		onTry(source, target) {
			const action = this.queue.willMove(target);
			const move = _optionalChain([action, 'optionalAccess', _52 => _52.choice]) === 'move' ? action.move : null;
	...
	tailwind: {
		...
		condition: {
			duration: 4,
			durationCallback(target, source, effect) {
				if (_optionalChain([source, 'optionalAccess', _53 => _53.hasAbility, 'call', _54 => _54('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 6;
				}
				return 4;
	...
	thunder: {
		...
		flags: {protect: 1, mirror: 1},
		onModifyMove(move, pokemon, target) {
			switch (_optionalChain([target, 'optionalAccess', _55 => _55.effectiveWeather, 'call', _56 => _56()])) {
			case 'raindance':
			case 'primordialsea':
				move.accuracy = true;
				break;
	...
	trickroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _57 => _57.hasAbility, 'call', _58 => _58('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
		...
		condition: {
			...
			onModifyMove(move, pokemon) {
				if (move.secondaries && move.id !== 'secretpower') {
					...
					if (_optionalChain([move, 'access', _59 => _59.self, 'optionalAccess', _60 => _60.chance])) move.self.chance *= 2;
				}
			},
		},
		secondary: null,
	...
	wideguard: {
		...
		condition: {
			...
			onTryHit(target, source, move) {
				...
				if (_optionalChain([move, 'optionalAccess', _61 => _61.target]) !== 'allAdjacent' && move.target !== 'allAdjacentFoes') {
					return;
				}
				if (move.isZ || move.isMax) {
	...
	wonderroom: {
		...
		condition: {
			duration: 5,
			durationCallback(source, effect) {
				if (_optionalChain([source, 'optionalAccess', _62 => _62.hasAbility, 'call', _63 => _63('persistent')])) {
					this.add('-activate', source, 'ability: Persistent', effect);
					return 7;
				}
				return 5;
