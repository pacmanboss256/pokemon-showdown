...
/** A single action that can be chosen. */
 ...
 class Side {
	...
	updateRequestForPokemon(pokemon, update) {
		if (!_optionalChain([this, 'access', _5 => _5.activeRequest, 'optionalAccess', _6 => _6.active])) {
	...
	clearChoice() {
		...
		if (this.battle.requestState === 'switch') {
			const canSwitchOut = this.active.filter(pokemon => _optionalChain([pokemon, 'optionalAccess', _7 => _7.switchFlag])).length;
