...
class Battle {
    constructor(options) {
        ...
        const format = options.format || dex_1.Dex.getFormat(options.formatid, true);
        this.format = format;
        ...
        for (const rule of this.ruleTable.keys()) {
            ...
            if (subFormat.exists) {
                const hasEventHandler = Object.keys(subFormat).some(val => val.startsWith('on') && !['onBegin', 'onValidateTeam', 'onChangeSet', 'onValidateSet'].includes(val));
                if (hasEventHandler)
    ...
    findEventHandlers(target, eventName, source) {
        ...
        if (target instanceof pokemon_1.Pokemon && target.isActive) {
    ...
    makeRequest(type) {
        ...
        const maxTeamSize = teamLengthData === null || teamLengthData === void 0 ? void 0 : teamLengthData.battle;
        if (type === 'teampreview') {
    ...
    getMaxTeamSize() {
        const teamLengthData = this.format.teamLength;
        return (teamLengthData === null || teamLengthData === void 0 ? void 0 : teamLengthData.battle) || 6;
    }
        ...
        switch (type) {
            case 'switch': {
                for (let i = 0; i < this.sides.length; i++) {
                    ...
                    const switchTable = [];
                    for (const pokemon of side.active) {
                        switchTable.push(!!(pokemon === null || pokemon === void 0 ? void 0 : pokemon.switchFlag));
                    }
            ...
            default: {
                for (let i = 0; i < this.sides.length; i++) {
                    const side = this.sides[i];
                    const activeData = side.active.map(pokemon => pokemon === null || pokemon === void 0 ? void 0 : pokemon.getMoveRequestData());
    ...
    switchIn(pokemon, pos, sourceEffect = null, isDrag) {
        ...
        const unfaintedActive = (oldActive === null || oldActive === void 0 ? void 0 : oldActive.hp) ? oldActive : null;
        if (unfaintedActive) {
    ...
    maybeTriggerEndlessBattleClause(trappedBySide, stalenessBySide) {
        if (this.turn <= 100 || !this.ruleTable.has('endlessbattleclause'))
            return;
        ...
        for (const side of this.sides) {
            ...
            for (const pokemon of side.pokemon) {
                berry = pokemon_1.RESTORATIVE_BERRIES.has(dex_1.toID(pokemon.set.item));
                if (['harvest', 'pickup'].includes(dex_1.toID(pokemon.set.ability)) ||
    ...
    boost(boost, target = null, source = null, effect = null, isSecondary = false, isSelf = false) {
        ...
        if (success && Object.values(boost).some(x => x > 0))
    ...
    getTarget(pokemon, move, targetLoc, originalTarget) {
        ...
        if (move.smartTarget)
            return this.getAtLoc(pokemon, targetLoc);
        ...
        if (move.target !== 'randomNormal' && this.validTargetLoc(targetLoc, pokemon, move.target)) {
            ...
            if ((target === null || target === void 0 ? void 0 : target.fainted) && target.side === pokemon.side) {
    ...
    runAction(action) {
        var _a, _b, _c;
        const pokemonOriginalHP = (_a = action.pokemon) === null || _a === void 0 ? void 0 : _a.hp;
        let residualPokemon = [];
        ...
        switch (action.choice) {
            ...
            case 'move':
                ...
                if (action.pokemon.fainted)
                    return false;
                this.runMove(action.move, action.pokemon, action.targetLoc, action.sourceEffect, action.zmove, undefined, action.maxMove, action.originalTarget);
                break;
            case 'megaEvo':
        ...
        if (this.gen < 5)
            this.eachEvent('Update');
        if (this.gen >= 8 && ((_c = this.queue.peek()) === null || _c === void 0 ? void 0 : _c.choice) === 'move') {
    ...
    forceSwitch(damage, targets, source, move, moveData, isSecondary, isSelf) {
    ...
    spreadMoveHit(targets, pokemon, move, moveData, isSecondary, isSelf) {
     ...
     */
    useMoveInner(move, pokemon, target, sourceEffect, zMove, maxMove) {
