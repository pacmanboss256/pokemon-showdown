...
class Pokemon {
    constructor(set, side) {
        ...
        this.baseAbility = dex_1.toID(set.ability);
        this.ability = this.baseAbility;
        this.abilityData = { id: this.ability };
        this.item = dex_1.toID(set.item);
    ...
    */
    ...
    allies() {
        ...
        if (this.battle.gameType === 'multi') {
            ...
            // @ts-ignore
            allies = this.battle.sides.flatMap((side) => side.n % 2 === team ? side.active : []);
        }
    ...
    foes() {
        ...
        if (this.battle.gameType === 'multi') {
            ...
            // @ts-ignore
            foes = this.battle.sides.flatMap((side) => side.n % 2 === team ? side.active : []);
        }
    ...
    getMoves(lockedMove, restrictData) {
        if (lockedMove) {
            lockedMove = dex_1.toID(lockedMove);
            this.trapped = true;
    ...
    getSwitchRequestData() {
        const entry = {
            ...
            moves: this.moves.map(move => {
                if (move === 'hiddenpower') {
                    return move + dex_1.toID(this.hpType) + (this.battle.gen < 6 ? '' : this.hpPower);
                }
    ...
    transformInto(pokemon, effect) {
        ...
        if (this.battle.gen === 4) {
            ...
            if (this.species.num === 493) {
                ...
                const targetForme = ((item === null || item === void 0 ? void 0 : item.onPlate) ? 'Arceus-' + item.onPlate : 'Arceus');
    ...
    disableMove(moveid, isHidden, sourceEffect) {
        ...
        for (const moveSlot of this.moveSlots) {
            if (moveSlot.id === moveid && moveSlot.disabled !== true) {
                ...
                moveSlot.disabledSource = ((sourceEffect === null || sourceEffect === void 0 ? void 0 : sourceEffect.fullname) || moveSlot.move);
            }
        }
    }
    ...
    setStatus(status, source = null, sourceEffect = null, ignoreImmunities = false) {
        ...
        if (this.status === status.id) {
            if (((_a = sourceEffect) === null || _a === void 0 ? void 0 : _a.status) === this.status) {
                this.battle.add('-fail', this, this.status);
            }
            else if ((_b = sourceEffect) === null || _b === void 0 ? void 0 : _b.status) {
        ...
        if (!ignoreImmunities && status.id &&
            ...
            if (!this.runStatusImmunity(status.id === 'tox' ? 'psn' : status.id)) {
                ...
                if ((_c = sourceEffect) === null || _c === void 0 ? void 0 : _c.status) {
    ...
    takeItem(source) {
        ...
        if (!source)
            source = this;
        if (this.battle.gen === 4) {
            if (dex_1.toID(this.ability) === 'multitype')
                return false;
            if (source && dex_1.toID(source.ability) === 'multitype')
                return false;
        }
        const item = this.getItem();
    ...
    hasItem(item) {
        ...
        if (!Array.isArray(item))
            return ownItem === dex_1.toID(item);
    ...
    hasAbility(ability) {
        ...
        if (!Array.isArray(ability))
            return ownAbility === dex_1.toID(ability);
    ...
    addVolatile(status, source = null, sourceEffect = null, linkedStatus = null) {
        ...
        if (!this.runStatusImmunity(status.id)) {
            ...
            if ((_a = sourceEffect) === null || _a === void 0 ? void 0 : _a.status) {
                this.battle.add('-immune', this);
            }
            return false;
        }
