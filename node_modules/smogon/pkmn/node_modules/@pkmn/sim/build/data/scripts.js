...
exports.Scripts = {
    ...
    trySpreadMoveHit(targets, pokemon, move) {
        ...
        if (move.spreadHit)
            this.attrLastMove('[spread] ' + hitSlot.join(','));
        return moveResult;
    },
    ...
    hitStepMoveHitLoop(targets, pokemon, move) {
        ...
        for (hit = 1; hit <= targetHits; hit++) {
            ...
            if (hit > 1 && pokemon.status === 'slp' && !isSleepUsable)
                break;
            if (targets.every(target => !target || !target.hp))
                break;
            move.hit = hit;
    ...
    spreadMoveHit(targets, pokemon, moveOrMoveName, moveData, isSecondary, isSelf) {
        ...
        if (moveData.forceSwitch)
            damage = this.forceSwitch(damage, targets, pokemon, move, moveData, isSecondary, isSelf);
        for (const i of targets.keys()) {
            if (!damage[i] && damage[i] !== 0)
                targets[i] = false;
        }
    ...
    secondaries(targets, pokemon, move, moveData, isSelf) {
        ...
        for (const target of targets) {
            if (target === false)
                continue;
            const secondaries = this.runEvent('ModifySecondaries', target, pokemon, moveData, moveData.secondaries.slice());
    ...
    canMegaEvo(pokemon) {
        ...
        const item = pokemon.getItem();
        // Mega Rayquaza
        if ((this.gen <= 7 || this.ruleTable.has('standardnatdex')) && (altForme === null || altForme === void 0 ? void 0 : altForme.isMega) && (altForme === null || altForme === void 0 ? void 0 : altForme.requiredMove) &&
    ...
    isAdjacent(pokemon1, pokemon2) {
        if (pokemon1.fainted || pokemon2.fainted)
            return false;
        if (pokemon1.side === pokemon2.side)
            return Math.abs(pokemon1.position - pokemon2.position) === 1;
