...
const adaptable_1 = require("@smogon/calc/adaptable");
const dex = __importStar(require("@pkmn/dex"));
const sim = __importStar(require("@pkmn/sim"));
...
for (const [pkg, Dex] of Object.entries(DATA)) {
    describe(`Generations (${pkg})`, () => {
        const gens = new index_1.Generations(Dex);
            ...
            it('#getHiddenPower', () => {
                ...
                for (let num = 3; num <= 7; num++) {
                    ...
                    for (const type of gen.types) {
                        if (['Normal', 'Fairy', '???'].includes(type.name))
                            continue;
                        expect(gen.types.getHiddenPower(Object.assign(Object.assign({}, ivs), type.HPivs)))
...
describe('@smogon/calc', () => {
    it('usage', () => {
        const gen = new index_1.Generations(dex.Dex).get(5);
        const result = adaptable_1.calculate(gen, new adaptable_1.Pokemon(gen, 'Gengar', {
            item: 'Choice Specs',
            nature: 'Timid',
            evs: { spa: 252 },
            boosts: { spa: 1 },
