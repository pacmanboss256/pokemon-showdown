...
class TeamValidator {
    ...
    validateSet(set, teamHas) {
        ...
        let item = dex.getItem(utils_1.Utils.getString(set.item));
        set.item = item.name;
        let ability = dex.getAbility(utils_1.Utils.getString(set.ability));
        set.ability = ability.name;
        let nature = dex.getNature(utils_1.Utils.getString(set.nature));
        ...
        if (ruleTable.has('obtainableformes') && (dex.gen <= 7 || this.format.id.includes('nationaldex'))) {
            ...
            else if (species.id === 'rayquaza' && set.moves.map(dex_1.toID).includes('dragonascent')) {
        ...
        for (const moveName of set.moves) {
            if (!moveName)
                continue;
            const move = dex.getMove(utils_1.Utils.getString(moveName));
            if (!move.exists)
    ...
    validateStats(set, species, setSources) {
        ...
        if (dex.gen <= 2) {
            ...
            if (set.species === 'Marowak' && dex_1.toID(set.item) === 'thickclub' &&
    ...
    findEggMoveFathers(source, species, setSources, getAll = false) {
        ...
        if (eggGroups[0] === 'Undiscovered')
            eggGroups = dex.getSpecies(species.evos[0]).eggGroups;
    ...
    validateForme(set) {
        ...
        if (species.name === 'Necrozma-Ultra') {
            ...
            if (species.requiredMove && !set.moves.includes(dex_1.toID(species.requiredMove))) {
            ...
            if (species.requiredItems && !species.requiredItems.includes(item.name)) {
                if (dex.gen >= 8 && (species.baseSpecies === 'Arceus' || species.baseSpecies === 'Silvally')) {
                    ...
                    problems.push(`${name} needs to hold ${species.requiredItems.join(' or ')} to be in its ${species.forme} forme.`, `(It will revert to its ${baseSpecies.baseForme} forme if you remove the item or give it a different item.)`);
                }
            }
            if (species.requiredMove && !set.moves.includes(dex_1.toID(species.requiredMove))) {
                const baseSpecies = this.dex.getSpecies(species.changesFrom);
    ...
    checkSpecies(set, species, tierSpecies, setHas) {
        ...
        setHas['basepokemon:' + dex_1.toID(species.baseSpecies)] = true;
        let isMega = false;
        ...
        const tierTag = 'pokemontag:' + dex_1.toID(tier);
        setHas[tierTag] = true;
        const doublesTier = tierSpecies.doublesTier === '(DUU)' ? 'DNU' : tierSpecies.doublesTier;
        const doublesTierTag = 'pokemontag:' + dex_1.toID(doublesTier);
        ...
        banReason = ruleTable.check('basepokemon:' + dex_1.toID(species.baseSpecies));
        if (banReason) {
            return `${species.name} is ${banReason}.`;
        }
        if (banReason === '') {
        ...
        if (tierSpecies.isNonstandard) {
            banReason = ruleTable.check('pokemontag:' + dex_1.toID(tierSpecies.isNonstandard));
            if (banReason) {
    ...
    checkItem(set, item, setHas) {
        ...
        if (item.isNonstandard) {
            banReason = ruleTable.check('pokemontag:' + dex_1.toID(item.isNonstandard));
            if (banReason) {
    ...
    checkMove(set, move, setHas) {
        ...
        if (move.isNonstandard) {
            banReason = ruleTable.check('pokemontag:' + dex_1.toID(move.isNonstandard));
            if (banReason) {
    ...
    checkAbility(set, ability, setHas) {
        ...
        if (ability.isNonstandard) {
            banReason = ruleTable.check('pokemontag:' + dex_1.toID(ability.isNonstandard));
            if (banReason) {
    ...
    checkNature(set, nature, setHas) {
        ...
        if (nature.isNonstandard) {
            banReason = ruleTable.check('pokemontag:' + dex_1.toID(nature.isNonstandard));
            if (banReason) {
            ...
            if (dex.gen <= 5 && eventData.abilities && eventData.abilities.length === 1 && !eventData.isHidden) {
                if (species.name === eventSpecies.name) {
                    ...
                    if (ability1.gen && eventData.generation >= ability1.gen) {
                        ...
                        const requiredAbilitySlot = (dex_1.toID(eventData.abilities[0]) === ability1.id ? 1 : 0);
    ...
    reconcileLearnset(species, setSources, problem, name = species.name) {
        const dex = this.dex;
        const problems = [];
        if (problem) {
        ...
        if (setSources.babyOnly && setSources.sources.length) {
            ...
            const babyEvo = dex_1.toID(baby.evos[0]);
    ...
    checkLearnset(move, s, setSources = this.allSources(s), set = {}) {
        const dex = this.dex;
        if (!setSources.size())
            ...
            if (lsetData.learnset[moveid] || lsetData.learnset['sketch']) {
                ...
                for (let learned of lset) {
                    ...
                    if (learnedGen <= moveSources.sourcesBefore)
                        continue;
                    if (learnedGen < 7 && setSources.isHidden && (dex.gen <= 7 || format.mod === 'gen8dlc1') &&
        ...
        if (!moveSources.size()) {
            if (this.minSourceGen > 1 && sometimesPossible)
                return { type: 'pastgen', gen: this.minSourceGen };
