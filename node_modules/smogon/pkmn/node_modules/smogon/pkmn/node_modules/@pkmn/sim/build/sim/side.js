...
class Side {
    ...
    chooseMove(moveText, targetLoc = 0, megaDynaOrZ = '') {
        ...
        if (typeof moveText === 'number' || (moveText && /^[0-9]+$/.test(moveText))) {
            ...
            if (!targetType && ['', 'zmove'].includes(megaDynaOrZ) && request.canZMove) {
                for (const [i, moveRequest] of request.canZMove.entries()) {
                    if (!moveRequest)
                        continue;
                    if (moveid === dex_1.toID(moveRequest.move)) {
                        moveid = request.moves[i].id;
                        targetType = moveRequest.target;
                        megaDynaOrZ = 'zmove';
                        break;
        ...
        if (maxMove)
            targetType = this.battle.dex.getMove(maxMove).target;
        // Validate targetting
        if (autoChoose) {
            targetLoc = 0;
        }
        ...
        if (lockedMove) {
            ...
            const lockedMoveID = dex_1.toID(lockedMove);
    ...
    chooseSwitch(slotText) {
        ...
        if (isNaN(slot) || slot < 0) {
            ...
            for (const [i, mon] of this.pokemon.entries()) {
                if (slotText.toLowerCase() === mon.name.toLowerCase() || dex_1.toID(slotText) === mon.species.id) {
                    slot = i;
                    break;
                }
            }
    ...
    clearChoice() {
        ...
        if (this.battle.requestState === 'switch') {
            const canSwitchOut = this.active.filter(pokemon => pokemon === null || pokemon === void 0 ? void 0 : pokemon.switchFlag).length;
            ...
            switch (choiceType) {
                case 'move':
                    ...
                    while (true) {
                        ...
                        if (/\s(?:-|\+)?[1-3]$/.test(data) && dex_1.toID(data) !== 'conversion2') {
