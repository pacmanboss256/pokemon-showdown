...
class ExhaustiveRunner {
    ...
    createPools(dex) {
        return {
            pokemon: new Pool(ExhaustiveRunner.onlyValid(dex.gen, dex.data.Pokedex, p => dex.getSpecies(p), (_, p) => (p.name !== 'Pichu-Spiky-eared' && p.name.substr(0, 8) !== 'Pikachu-')), this.prng),
            ...
            moves: new Pool(ExhaustiveRunner.onlyValid(dex.gen, dex.data.Moves, m => dex.getMove(m), m => (m !== 'struggle' && (m === 'hiddenpower' || m.substr(0, 11) !== 'hiddenpower'))), this.prng),
        };
    }
    logProgress(p) {
    ...
    static getSignatures(dex, pools) {
        ...
        for (const id of pools.items.possible) {
            ...
            if (item.megaEvolves) {
                const pokemon = dex_1.toID(item.megaEvolves);
                const combo = { item: id };
                ...
                for (const user of item.itemUser) {
                    const pokemon = dex_1.toID(user);
                    const combo = { item: id };
                    if (item.zMoveFrom)
                        combo.move = dex_1.toID(item.zMoveFrom);
                    let combos = signatures.get(pokemon);
                    if (!combos) {
                        combos = [];
                        signatures.set(pokemon, combos);
...
class CoordinatedPlayerAI extends random_player_ai_1.RandomPlayerAI {
    ...
    choosePokemon(choices) {
        ...
        for (const { slot, pokemon } of choices) {
            const species = dex_1.toID(pokemon.details.split(',')[0]);
    ...
    markUsedIfGmax(active) {
        if (active && !active.canDynamax && active.maxMoves && active.maxMoves.gigantamax) {
            this.pools.pokemon.markUsed(dex_1.toID(active.maxMoves.gigantamax));
        }
    }
}
